...
int main(int argc, char* argv[])
{
    ...
    int data_dimension = (int) atoi(argv[4]);
    std::cout << "Data dimension is " << data_dimension << "\n";

    std::string experiment_result_folder = argv[3];
    std::cout << "Results folder is " << experiment_result_folder << "\n";

    std::cout << "Read Ground Truth Volume " << argv[1] << "\n";
    volume_ptr<uint32_t> gt_ptr = read_volume<uint32_t>(argv[1], data_dimension);

    std::cout << "Read Affinity Volume " << "\n";
    affinity_graph_ptr<float> aff = read_affinity_graph<float>(argv[2], data_dimension, data_dimension, data_dimension);
    ...
    if ( 1 )
    {
        ...
        for ( std::size_t thold = 200; thold <= 100000; thold += 1000 )
        {
...

            {
                ...
                // write_volume("experiments/linear/" + std::to_string(thold) + ".dat", seg);

                auto x = compare_volumes(*gt_ptr, *seg, 256);
                r.push_back(x.first);
                r.push_back(x.second);
            }
            write_to_file(experiment_result_folder + "/linear.dat", r.data(), r.size());
        }
    ...
    if ( 1 )
    {
        ...
        for ( std::size_t thold = 200; thold <= 100000; thold += 1000 )
        {
...

            {
                ...
                // write_volume("experiments/square/" + std::to_string(thold) + ".dat", seg);

                auto x = compare_volumes(*gt_ptr, *seg, 256);
                r.push_back(x.first);
                r.push_back(x.second);
            }
            write_to_file(experiment_result_folder + "/square.dat", r.data(), r.size());
        }
...
//        return 0;
    ...
    {
        std::vector<double> r;
        for ( std::size_t thold = 100; thold <= 50000; thold += 1000 )
        {
            if ( thold > 1000 ) thold += 900;
            if ( thold > 10000 ) thold += 9000;
        ...
        write_to_file(experiment_result_folder + "/felzenszwalb.dat", r.data(), r.size());
    }
    ...
    if ( 1 )
    {
        ...
        for ( std::size_t thold = 100; thold <= 50000; thold += 1000 )
        {
...

            {
                ...
                // write_volume("experiments/threshold/" + std::to_string(thold) + ".dat", seg);
        ...
        write_to_file(experiment_result_folder + "/threshold.dat", r.data(), r.size());
    }
