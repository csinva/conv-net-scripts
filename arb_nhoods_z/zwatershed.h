...
std::map<std::string,std::list<float>> zwshed_initial_c(int dx, int dy, int dz, uint32_t*node1,
...
std::map<std::string,std::vector<double>> merge_no_stats(int dx,int dy, int dz, float * rgn_graph,
                                        int rgn_graph_len, uint32_t * seg_in, uint32_t*counts, int counts_len, int thresh);
