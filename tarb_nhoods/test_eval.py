...
threshes = [500, 1000, 2000, 10000]  # for i in range(1,6)]+[i*20000 for i in range(2,16)] # 100...1,000...100,000
iters = [10000]
strs = ["2"]
...
print "num segs gt,seg", len(np.unique(gt.flatten())), max(seg_cc.flatten())
segs, seg_sizes = tw.marker_watershed(seg_cc.flatten(), node1, node2, edge_affs, [0, 10, 100, 1000, 10000, 100000])
seg = segs[0]
